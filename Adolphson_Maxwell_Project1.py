...
class PhysicsCalcKine(object):
	...
	def options(self):
		print(
			"This function will prefrom some basic Kinematic functions commonly found in introductory physics courses.")
		print(
			"---------------------------------------------------------------------------------------------------------")
...
class PhysicsRotCalc(PhysicsCalcKine):
	...
	def options(self):
		...
		print(
			"---------------------------------------------------------------------------------------------------------------------")
		print(
			"This function will prefrom some basic Rotational Kinematic functions commonly found in introductory physics courses.")
		print(
			"---------------------------------------------------------------------------------------------------------------------")
		...
		if (self.fx == 3):
			...
			if (self.choice == 4):
				print(
					"Please enter in the mass (kg), radius (m) and angular acceleration (m/s^2) of the hollow sphere:")
				self.m = raw_input("Mass = ")
				self.length = raw_input("Radius is = ")
				self.angularA = raw_input("Angular acceleration = ")
...
class PhysicsMathCalc(object):
	...
	def options(self):
		...
		if (self.choice == 1):
			...
			print(
				"This program prefroms Numeric Integration of a third order polynomial using the trapzoid rule.\nThis function requires the four constant coefficents of a cubic function, and the ")
	...
	def NumericIntegration(self):
		...
		while (counter < intervals):
			...
			sum_ += 2.0 * ((self.a * self.lowerLim ** 3) + (self.b * self.lowerLim ** 2) + (
						self.c * self.lowerLim) + self.d)
	...
	def printResults(self):
		if (self.choice == 1):
			...
			print("The zero of %sx^3 + %sx^2 + %sx + %s around the point %s is located at X= %s" % (
			self.a, self.b, self.c, self.d, self.intercept, round(self.results, 4)))
		elif (self.choice == 2):
			# Prints the results of the numeric integration function.
			print("The approimate area under the curve %sx^3 + %sx^2 + %sx + %s is %s" % (
			self.a, self.b, self.c, self.d, self.results))
		elif (self.choice == 3):
...
if __name__ == "__main__":
	...
	while (key):
		...
		print(
			"-----------------------------------------------------------------------------------------------------------------------------------")
		print(
			"(1) [Kinematic Calculator] (2) [Rotational Kinematic Calculator] (3) [Physics Math Calculator] (4) [Satistics Calculator] (5) Exit")
		print(
			"-----------------------------------------------------------------------------------------------------------------------------------")
		...
		print(
			"If Statistics Calculator  (4) is selected the progrm will execute automatically, reading in the date from")
